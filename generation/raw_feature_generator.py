...
class RawFeatureGenerator:
    ...
    def export_to_csv(self, filename="../data/raw_features.csv", n=1000):
        df = pd.DataFrame(self.generate_batch(n))
        df.to_csv(filename, index=False)
        print(f"📦 Exported {n} raw records to {filename}")
